...
with open(deletions_file, 'r') as dels:
    for l in dels:
        ...
        try:
            ...
            (longest_hom, mhseq, homseq, deletion_size, deleted_bases, insertion_size, inserted_seq, templated_up, templated_down, templated_insertion_size, mechanism, seq1, seq2) = run_script(position, None, split_read, genome, '', '', ori, cut)
        except ValueError:
            cut = 200
            (sample, length, split_read, position) = parts
            print(position, split_read, cut)
            (longest_hom, mhseq, homseq, deletion_size, deleted_bases, insertion_size, inserted_seq, templated_up, templated_down, templated_insertion_size, mechanism, seq1, seq2) = run_script(position, None, split_read, genome, '', '', ori, 200)
